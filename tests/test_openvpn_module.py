...
""",
            ...
            self.assertIn(".site-header{position:sticky;z-index:50;top:0", css)
            mobile_css = css.split("@media(max-width:760px)", 1)[1].split(
                "@media(max-width:480px)", 1
            )[0]
            self.assertIn(
                ".site-header{position:sticky;top:0;padding-top:7px}",
                mobile_css,
            )
            self.assertNotIn(".site-header{position:relative", mobile_css)
    ...
    def test_dynamic_banner_uses_authenticated_pam_account_delivery(self):
        ...
        if "PAYLOAD_SSH_AUTH_SESSION" in installer:
            self.assertIn(
                "tdz_ssh_auth_session.py|$PAYLOAD_SSH_AUTH_SESSION", installer
            )
            self.assertLess(
                installer.index(
                    'install -m 755 "$PAYLOAD_SSH_AUTH_SESSION" '
                    '"$TARGET_SSH_AUTH_SESSION"'
                ),
                installer.index(
            ...
            # Packed-release installers migrate any older source helper before
            # the authenticated runtime installs its validated replacement.
            self.assertIn("sanitize_tdz_pam_hooks", installer)
            self.assertIn('index($0, "tdz_ssh_auth_session.py")', installer)
            self.assertIn("sync_runtime", installer)
